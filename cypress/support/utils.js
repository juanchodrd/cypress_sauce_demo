...
cy.get(select).find(opciones).then(($options) => {
const randomOption = $options.get(Math.floor(Math.random() * $options.length-1));// se resta 1 para excluir la opción predeterminada "Seleccione Localidad"
...
export function eliminarItems(body,elementosBuscados) {
  cy.get(body).then($body => {
      if ($body.find(elementosBuscados).length > 0) {
          ...
          cy.get($body).find(elementosBuscados)  // Busca los elementos <button> con la clase específica          
          .each(($button) => {  // Itera sobre cada botón
